...
- **家長幹部**已是真實名單 ✓（K260702：會長趙芃＋副會長群兼任各組組長共七組；`officers.js` 拆 `president`／`officerGroups`，關於我們頁改**階層式卡片**——會長卡置頂＋短接線＋「副會長群（兼任各組組長）」說明＋各組卡片（組名 chip＋名字置中）；招生組虛線卡「熱情招募中」；謝伊婷兼總務組與社群小編組、依使用者選擇各佔一卡）。
- **待補真實素材**：
