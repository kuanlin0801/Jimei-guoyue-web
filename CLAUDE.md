...
- **介面細節優化（redesign 審查前四項）** ✓（K260704）：①首頁樂器輪播圖 PNG→WebP（3.3MB→0.7MB，`public/instruments/`），且只先載第一張、其餘輪播啟動前才補 `src`（`data-src`；reduced-motion 完全不多載）；②公告／近期行程／文件／支援看板的「載入中…」改 skeleton 佔位卡（共用樣式在 `global.css` 的 `.skel*`＋`.sr-only`，文件／支援頁再各自微調）；③自訂 404 頁（`src/pages/404.astro`；`wrangler.jsonc` 加 `not_found_handling: "404-page"` 才會回 Astro 的 404.html）；④微互動——`.btn`／報名鈕按壓回饋、導覽 hover 過渡、手機漢堡三線轉 X＋下拉淡入（皆帶 reduced-motion 關閉）；⑤紙紋理背景（K260704B）——`global.css` 的 `html` 鋪極淡暖棕 SVG 噪點（inline data URI、feTurbulence，alpha 0.07；白卡不受影響），使用者從 7%／12%／宣紙纖維紋對比中選定 7%。審查報告其餘待辦：朱印章 SVG、`clamp()` 字級、skip link＋`:focus-visible`、`100dvh`。
