...
- **暑訓作業繳交**（接上 Google 表單）✓（K260708A）：`/summer` 底部「作業繳交」卡接上 Google 表單「檔案上傳」題——家長填姓名／分部／曲目＋上傳演奏影片，回應試算表看繳交紀錄＋影片連結；表單掛站方個人 Google 帳號（5TB，容量非問題）。按鈕 `target="_blank"` 開新分頁到官方表單頁：含「檔案上傳」題的表單**不宜 iframe 內嵌**（登入／上傳會被現代瀏覽器擋第三方 cookie），故刻意不做同頁彈窗。網址存 `src/data/summer.js` 的 `uploadFormUrl`，填了按鈕即亮、留空顯示「建置中」。⚠️ 表單端維運（網站無法代勞）：「限回覆 1 次」須**關**（桃花鄉／黑黑天兩首、兩期限，家長分兩次交），回應試算表＋影片資料夾要**共用給子玉／蔡指揮檢視**否則老師收不到繳交。
