...
## 目錄結構
- `src/pages/*.astro` — 各頁：index（首頁）、calendar、announcements、support（活動支援）、about（關於我們）、documents、gallery（相簿）、achievements（成果）、intro（樂團介紹翻頁書）、summer（暑訓專區）、duty（值班須知）、homework（作業繳交規範）
...
- `src/data/*.js` — 半靜態內容資料檔：club、teachers、officers、albums、achievements、support-events、summer、duty、homework
...
- **環境變數有兩區，名字幾乎一樣、別搞混**（K260703 上線時就設錯過一次）：`PUBLIC_*`（Astro build 時嵌入）設在 **Settings → Build → Variables and secrets**（build 區），改完需重新 push 觸發新 build 才生效；密碼閘的 `SITE_PASSWORD`／`COOKIE_SECRET` 設在 **Settings → Variables and Secrets**（runtime 區，有 worker script 後已解鎖），改完**即刻生效**不用重新部署。設錯到 build 區的症狀＝輸入正確密碼一直顯示「密碼不正確」。以下皆為 build 區的 `PUBLIC_*`：
...
- **介面細節優化（redesign 審查前四項）** ✓（K260704）：①首頁樂器輪播圖 PNG→WebP（`public/instruments/`），且只先載第一張、其餘輪播啟動前才補 `src`（`data-src`；reduced-motion 完全不多載）。⚠️ K260704A 初版用有損 q82 導致水彩色暈出現可見色塊，K260704C 改 **nearLossless q60** 重轉（3.3MB→1.4MB，maxΔ=4 不可辨）——此類低對比暈染去背圖**不能用有損 WebP**（即使 q95＋smartSubsample 仍 13% 像素偏差>4），要嘛 nearLossless 要嘛 lossless。K260706C 又清掉六張圖自帶的**去背殘渣**（每張數千至兩萬顆 alpha 2–15% 的綠色噴點，成片看似霉斑「不乾淨」；殘渣 alpha 全在 39 以下、真實內容與抗鋸齒邊緣都在 80 以上，把 alpha<40 歸零即可安全清除）。K260706D **最終版**：改用家長提供的原始完整插畫（`Reference/主頁底圖/images/` 無色斑版，2176×1632 **不透明** PNG）重做——每張採四角平均色算出 1–6 階偏移把底色對齊 `--paper` #FBF6EC、縮 1600×1200、**有損 WebP q92**＋smartSubsample（不透明全圖無「壓縮圖 vs 純色底」合成交界，有損不會露餡；nearLossless 只有去背圖才需要）、總量 676KB 比 near-lossless 版再省 44%。同資料夾上層另有「有色斑版」原稿（帶粉紅／灰綠／杏色水彩圓斑，經 preview 比對後使用者選無色斑版）；②公告／近期行程／文件／支援看板的「載入中…」改 skeleton 佔位卡（共用樣式在 `global.css` 的 `.skel*`＋`.sr-only`，文件／支援頁再各自微調）；③自訂 404 頁（`src/pages/404.astro`；`wrangler.jsonc` 加 `not_found_handling: "404-page"` 才會回 Astro 的 404.html）；④微互動——`.btn`／報名鈕按壓回饋、導覽 hover 過渡、手機漢堡三線轉 X＋下拉淡入（皆帶 reduced-motion 關閉）；⑤紙紋理背景（K260704B 加入、K260704C **移除**，勿再提案）——極淡 SVG 噪點鋪 `html` 底看似無害，但首頁 hero 的純色 scrim 與樂器「色暈圖」自帶的水彩暈都會蓋住噪點，在紋理底上形成肉眼可見的「平滑區 vs 顆粒區」色塊邊界（開關對照實測確認）；此設計的色暈插畫本質上要純色底。朱印章 SVG 落款經預覽後**使用者否決**（K260704，勿再提案）。⑥收尾三項（K260704D）——`clamp()` 流動字級（`h1` 28–34px、hero 大標 54–80px，移除斷點硬切）、skip link（`Layout.astro`，`<main id="main">`）＋品牌綠 `:focus-visible`、`body` `100dvh`（含 `100vh` fallback）。**redesign 審查項目至此全部完結**。暑訓專區頁再追加（K260704E）：作業卡 ≥860px 改左文右影 grid（影片固定 400px、`grid-row:1/span 3` 垂直置中，頁長近乎減半）、行事曆圖補 `width/height` 防 CLS、badge 分色（`考核`＝朱紅 `.exam`、繳交＝綠，依 badge 文字開頭判斷）。可選未做：YouTube facade（縮圖點擊才載 player）、作業繳交升級為每張作業卡各一顆繳交按鈕（目前為底部單一按鈕，見下則）。
- **暑訓作業繳交**（接上 Google 表單）✓（K260708A）：`/summer` 底部「作業繳交」卡接上 Google 表單「檔案上傳」題——家長填姓名／樂器／曲目＋上傳演奏影片，回應試算表看繳交紀錄＋影片連結；表單掛站方個人 Google 帳號（5TB，容量非問題）。按鈕 `target="_blank"` 開新分頁到官方表單頁：含「檔案上傳」題的表單**不宜 iframe 內嵌**（登入／上傳會被現代瀏覽器擋第三方 cookie），故刻意不做同頁彈窗。網址存 `src/data/summer.js` 的 `uploadFormUrl`，填了按鈕即亮、留空顯示「建置中」。⚠️ 表單端維運（網站無法代勞）：「限回覆 1 次」須**關**（桃花鄉／黑黑天兩首、兩期限，家長分兩次交）。**作業繳交已接自動通知老師** ✓（K260711A 實測通過）：家長送出表單即由 Apps Script `onFormSubmit`（`apps-script/homework-notify.gs`，裝在站方帳號的回應試算表上）自動寄含姓名／樂器／曲目／影片連結的信給老師，老師不必再開試算表查看；欄位動態讀表頭，表單日後增減／改題目不必改 script。收件老師 email 寫在該檔第一行 `TEACHERS`（換老師要去 Google 端 Apps Script 改，非改 repo）。⚠️ 影片存取權依繳交方式：走 **YouTube 連結**免共用（家長設「不公開 Unlisted」即可，勿設私人），走 **Drive 影片上傳**才需把表單上傳資料夾分享給老師。
- **值班須知頁** ✓（K260706A）：`/duty` 一頁合併「值班注意事項」（4 點編號卡＋冷氣卡／遙控器位置照）與「樂器擺放位置」（樂器室 1／國樂 2／揚琴／木琴鐵琴／譜架五組卡＋黏鼠板警示條＋孩童勿奔跑叮嚀卡）；文案集中 `src/data/duty.js`、照片在 `public/duty/`（由 `Reference/值班與樂器擺放注意事項/` 的 Word 截圖自動偵測照片區塊裁切轉 WebP，去掉截圖標題與選取控點）。入口＝首頁 hero「暑訓專區」旁「值班須知」鈕，**不進導覽列**（七項已滿，使用者決定只放首頁）。⚠️ 值班照片圈起的白色機器依外觀判斷標為「冷氣遙控器」，尚未經幹部證實。
- **作業繳交規範頁** ✓（K260810A）：`/homework` 收蔡指揮訂的影片作業通則，分「拍攝規範」（畫面鏡頭／環境與畫質／伴奏／替代樂器）與「繳交與審核」（影片長度／繳交與修正時限／審核與提醒機制）兩區，各項一張卡（`repeat(auto-fit,minmax(300px,1fr))`，桌機三欄），底部「要去哪裡交？」卡連回暑訓專區。文案集中 `src/data/homework.js`（每項為 `{title, points:[]}`，points 一條渲染成段落、多條渲染成清單）。入口＝①首頁 hero 第三顆鈕「作業規範」（暑訓專區／作業規範／值班須知，360px 窄機自動換行成 2+1）②`/summer` 作業繳交卡上方的「⚠️ 錄影前請先看」連結；比照 `/duty` **不進導覽列**（七項已滿）。**刻意不放公告**：公告內文是 CSV 單欄渲染成一個 `<p>`、無 `white-space:pre-line`，七段規範會被壓成一坨連續文字。⚠️ 兩處待幹部向老師確認（已在 `homework.js` 留註解）：①原文「請同步於訊息文字中附上 BPM」是 LINE 用語，站上走 Google 表單，已改為不綁管道的寫法，**但表單需有備註欄或加一題「節拍器速度」老師才收得到**；②「長度控制在 30 秒內」與暑訓的整首跟奏作業（《桃花鄉》《黑黑天》）對不上，需請指揮補一句區分訂正片段 vs 整首跟奏。
...
- **首頁重要活動倒數卡** ✓（K260727，`/` 右欄「近期行程」上方；手機版置最上）：幹部在**任一日曆的活動標題加 `★`**（也認 `⭐`／`【重要】`，文件只教 `★`），首頁就把最近一場拉成大卡——左側活動名稱＋日期、右側 62px 倒數天數（`還有 26 天`／`明天`／`就是今天`），其餘重要活動壓成小字列，合計上限 3 筆。標記字元顯示前一律移除，只有 Google 端與行事曆嵌入畫面看得到 `★`。純邏輯在 `events.js`（`isFeatured`／`stripMarker`／`pickFeatured`／`daysUntil`／`formatCountdown`）＋vitest，天數用**台北日界線**算（同一場活動在一天中任何時刻查看都同一個數字）。與「近期行程」共用**同一次** Calendar API 請求，未新增請求。設計 `docs/superpowers/specs/2026-07-27-home-featured-countdown-design.md`、計畫 `docs/superpowers/plans/2026-07-27-home-featured-countdown.md`。
  - ⚠️ **幹部使用須知**（網站無法代勞，需口頭交代）：①**週期性活動**標 `★` 只會顯示最近一次（`singleEvents=true` 會展開成每次各一筆，已依標題去重，否則三個欄位會被同一活動佔滿）；②**跨多天的全天活動**請標在**第一天**——`isUpcoming` 只看開始日，多天活動進入第二天後就會從倒數卡消失；③**有指定時間的活動**一過開始時間就從卡片消失（換下一場），故「就是今天」通常只在活動開始前看得到；④取消強調把 `★` 刪掉即可，活動過了會自動換下一場、不必回頭清理。
  - ⚠️ **抓取範圍限制**：每個日曆抓未來 **50 筆**（原 10 筆）。暑訓期間課表密集時約涵蓋一個多月；若重要活動排在更遠的未來會抓不到，症狀是**卡片安靜地不出現**（不破版、也不報錯）。不夠用就調高 `fetchCalendarEvents` 的 `maxResults`。
  - ⚠️ **版面陷阱**（K260727H 修過一次，勿再犯）：`.home-cols` 的左欄 `grid-area:ann` **跨兩列**，若不指定 `grid-template-rows:min-content 1fr`，左欄多出來的高度會被分配進倒數卡那一列，把「近期行程」往下推（實測公告長時推 837px；**沒有 `★` 活動、卡片 `display:none` 時最嚴重，整個右欄掉到頁面最底**）。改這區塊的 grid 時務必用「左欄很長 × 卡片顯示／隱藏」四種組合驗證。
...
- **後援會幹部**（原稱家長幹部，K260702C 改名）已是真實名單 ✓（K260702：會長趙芃＋副會長群兼任各組組長共七組；`officers.js` 拆 `advisors`／`president`／`officerGroups`，關於我們頁改**階層式卡片**——榮譽顧問合併一卡（李美燕、邱凱圳，K260703F 新增，與會長卡同寬、置最頂）＋會長卡＋短接線＋「副會長群（兼任各組組長）」說明＋各組卡片（組名 chip＋名字置中）；招生組虛線卡「熱情招募中」；謝伊婷兼總務組與社群小編組、依使用者選擇各佔一卡）。
- **待補真實素材**：
